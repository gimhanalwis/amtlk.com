...
if(isset($_POST['search']))
        ...
        $sql = 'SELECT * FROM products WHERE CONCAT(`Name`, `Model`, `Manufacturer`, `Category`,`Price`,`Description`) LIKE "%'.$valueToSearch.'%"';
            ...
            $sql = 'SELECT * FROM products where Manufacturer="'.$man.'" ORDER BY Id DESC';
        }
        else if(isset($cat)){
...
<html lang="en">
  ...
  <head>
    ...
    <meta name="keywords" content="Anuramotor,Anura Motor trading,Anura Motors Trading,Gelioya,Srilanka,Autoparts,Product">

    <title>Anura Motor Trading(Pvt)Ltd-Products</title>
...
//end visitor record      
  ...
  <div class="dropdown-menu" aria-labelledby="dropdownMenu2" style="height: auto;
    max-height: 200px;
    overflow-x: hidden;">
...
$result2 = mysqli_query($conn, $sqln);
        ...
        if (mysqli_num_rows($result2) > 0) {
            ...
            <div class="col-lg-4 col-md-6 mb-4 text-uppercase">
              <div class="card">
                ...
                <div class="card-body">
                  ...
                  <?php if($row["Views"]==NULL){$row["Views"]=0;}?>
                  <h6 class="text-muted d-inline"style="font-size:11px;"><i class="fa fa-tachometer"></i>&nbsp&nbsp<?php  echo $row["Views"]; ?></h6> 
                </div>
                
              <h6><a href="product.php?id=<?php echo $row["Id"]; ?>" class="btn btn-primary">View Details</a></h6>
            </div> 
              </div>
...
if(isset($_REQUEST['Category'])){
  ...
  echo '<li><a class="page-link" href="products.php?Category='.$cat.'&&page=' . $page . '">' . $page . '</a></li> ';
}   
}
elseif(isset($_REQUEST['Manufacturer'])){
   for ($page=1;$page<=$number_of_pages;$page++) {
  echo '<li><a class="page-link" href="products.php?Manufacturer='.$man.'&&page=' . $page . '">' . $page . '</a></li> ';
}   
}
else{
   for ($page=1;$page<=$number_of_pages;$page++) {
  echo '<li><a class="page-link" href="products.php?page=' . $page . '">' . $page . '</a></li> ';
}   
}
